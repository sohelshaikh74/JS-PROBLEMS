...
const response = {
    ...
    data: {
        users: [
            { id: 1, name: "Aman", email: "aman@test.com", password: "123456" },
            { id: 2, name: "Aman", email: "aman@test.com", password: "abcdef" }
        ]
    }
}
//#1.API response transformation
//1.remove the password field from each user
// response.data = response.data.users.map((ele) => ({ id: ele.id, name: ele.name, email: ele.email }));

//2.return new array
// const newArr = response.data.users.map((ele) => ({ id: ele.id, name: ele.name, email: ele.email }));

//3.add new property isActive:true to each user
let isActiveProperty = response.data.users.map((ele) => ({ id: ele.id, name: ele.name, email: ele.email, isActive: true }));
// console.log(response)
let senitizedUsers = response.data.users.map(({ password, ...rest }) => ({ ...rest, isActive: true }));
// console.log(senitizedUsers);

// #2. Deep Nested Property Safe Access (Like Production)
const orders = {
    id: "ORD123",
    customer: {
        profile: {
...
//1.safely get city without crashing if any key is  missing
// const city = orders?.customer?.profile?.address?.city;
// console.log(city)

//2. provide default value"Unkown" if city does exist
// const { customer: { profile: { address: { city = "Unkown" } } } } = orders;  ===> this is not ideal way
// const city = orders?.customer?.profile?.address?.city ?? "Unknown"  //(if city does not exit)
// console.log(city);

//3.write usable function
function getNested(obj, path) {
    return path
        .split(".")
        .reduce((acc, key) => acc?.[key], obj);
}
console.log(getNested(orders, "customer.profile.address.city"))